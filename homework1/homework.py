...
system_prompt = """
You are a friendly assistant for a company's database.
- If the user asks a database question, generate SQL and return JSON:
  { "sql": "<SQL_QUERY>" }
- If the input is casual (greetings, general chat, unclear questions), respond naturally and politely.
- Ask clarifying questions if needed to eventually get a SQL query.
Database tables: Categories, Customers, Employees, Shippers, Suppliers, Products, Orders, OrderDetails.
"""



# ------------------- GRADIO CHAT FUNCTION -------------------
def chatbot_gradio(user_input):
    full_message = f"{system_prompt}\nUser query: {user_input}"
    response = chat.send_message(message=full_message)
    response_text = response.text.strip()

    # Try to parse JSON for SQL
    sql_query = None
    try:
        json_text = re.search(r'\{.*\}', response_text, re.DOTALL).group()
        data_json = json.loads(json_text)
        sql_query = data_json.get("sql")
    except Exception:
        sql_query = None
    ...
    if sql_query:
        ...
        result_text = response_text  # natural fallback
...
iface = gr.Interface(
    fn=chatbot_gradio,
    inputs=gr.Textbox(
        label="Ask something about the database", 
        lines=3, 
        placeholder="Type your question here..."
    ),
    outputs=gr.Textbox(
        label="Chatbot Response", 
        lines=20,  # taller output box
        max_lines=40,  # allows scrolling if needed
        placeholder="The chatbot response will appear here..."
    ),
    title="Northwind Database Chatbot",
    description="Ask questions about the Northwind database in natural language. The bot can generate SQL queries and return results in JSON."
)
